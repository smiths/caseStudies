...
tank. Given the appropriate inputs, SWHS predicts the temperature and thermal energy histories for the water.
...
the instance model (ODE) that models the solar water heating tank.% with PCM.
...
\item[A\refstepcounter{assumpnum}\theassumpnum \label{A_hcoeff}:] All heat
  transfer coefficients are constant over time.
...
has no spatial variation; that is, it is
  constant over the entire volume [\lcref{LC_tcoil}].
...
%and PCM 
  ...
  is less than (or
  equal to) the temperature of the coil [\lcref{LC_charge}].
...
\item[A\refstepcounter{assumpnum}\theassumpnum \label{A_OpRange}:] The operating
  ...
  above its boiling point temperature.

\item[A\refstepcounter{assumpnum}\theassumpnum \label{A_htank}:] The tank is
  perfectly insulated so that there is no heat loss from the tank [
  \lcref{LC_htank}].
...
% or the PCM
  ...
  heat generation per unit volume is zero.%, \iref{epcm}].
...
\item[A\refstepcounter{assumpnum}\theassumpnum \label{A_tk_prssr}:] The pressure
  ...
  $0^o\text{C}$ and $100^o\text{C}$, respectively.
...
\begin{tabular}{| p{\colAwidth} | p{\colBwidth}|}
  ...
  \hline
  Description &
  This General definition requires \aref{A_mixed}, \aref{A_const_density} and \aref{A_const_C}.\\
  & The basic equation governing the rate of change of temperature,
...
\hline
Description & 
This Data Definition requires \aref{A_Newt_coil}, \aref{A_tcoil} and \aref{A_tlcoil}.\newline
...
\begin{tabular}{| p{\colAwidth} | p{\colBwidth}|}
  ...
  \hline
  Description&
  This Instance Model requires \aref{A_tk_prssr}.\\
  &$T_W$ is the water temperature (\si{\celsius}).\\
...
\begin{tabular}{| p{\colAwidth} | p{\colBwidth}|}
  ...
  Description &
  This Instance Model requires \aref{A_tk_prssr}.\newline
  The above equation is derived using \tref{T_SHE}.  $E_W$ is the 
