...
\noindent \begin{longtable*}{l l p{12cm}} \toprule
  ...
  %                                                                  when melting begins
  % \\ 
  $E$ & \si[per-mode=symbol] {\joule} & change in heat energy
  \\ 
...
\item[A\refstepcounter{assumpnum}\theassumpnum \label{A_OpRange}:] The operating
  temperature range of the system is such that the material (water in this case)
  is always in a liquid state.  That is, the temperature will not drop below the
  melting point of water, or rise above its boiling point temperature
  [\tref{T_SHE}, \iref{ewat}, \iref{I_HWAT}].
...
% or the PCM
  ...
  heat generation per unit volume is zero [\iref{ewat}].%, \iref{epcm}].
...
\begin{tabular}{| p{\colAwidth} | p{\colBwidth}|}
  ...
  \hline
  Label&\bf Sensible heat energy (No State Change)\\
  \hline
  Equation&  
  $
  E = 
  ...
  where a phase change occurs, as assumed in~\aref{A_OpRange}.
  \\
  \hline
  Source &
