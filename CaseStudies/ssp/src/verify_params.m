...
if length(piez) > 0
    ...
    if length(piez) < 2
        error('Input Error : Piezometric surface must be described by at least 2 vertices')
    end
...
for i = 1:2
    if Xetr(i) < strat_init || Xetr(i) > strat_fin
        error('Input Error : Given bound on x of slip surface entry %0.1f is not between minimum slope x of %0.1f and maximum slope x of %0.1f', Xetr(i), strat_init, strat_fin)
    end
    if Xext(i) < strat_init || Xext(i) > strat_fin
        error('Input Error : Given bound on x of slip surface exit %0.1f is not between minimum slope x of %0.1f and maximum slope x of %0.1f', Xext(i), strat_init, strat_fin)
    end
end
