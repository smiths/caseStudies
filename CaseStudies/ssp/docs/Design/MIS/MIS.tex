...
\begin{tabularx}{\textwidth}{p{3cm}p{2cm}X}
	...
	12/09/18 & 1.3 & Finished updates based on Dr.~Smith's feedback\\
	\bottomrule
\end{tabularx}
...
\bmac{I also changed the remaining types to local types}
...
\begin{itemize}
	...
	\begin{itemize}
		...
		0))$ $\lor |slope.strat| < 2$ $\Rightarrow$ badSlopeGeometry\\
		...
		\lor |piez.piez| = 1$ $\Rightarrow$ badPiezGeometry\\
...
\bmac{Moved types to local types based on discussion in GitHub}
...
\begin{longtable}{p{0.14\textwidth} c p{0.8\textwidth}}
	...
	coords, wt : $\mathbb{R}$, slicedSlip : coords) \\
	slips &=& [slip] \\
\end{longtable}
...
\begin{center}
	...
	\begin{tabular} {p{0.18\textwidth}  p{0.15\textwidth}  p{0.15\textwidth} 
		...
		kin\_adm & coords & $\mathbb{B}$ & - \\ \hline
...
\begin{itemize}
	...
	\begin{itemize}
		...
		\in [0..|\textit{slip\_surf}| - 2] : 
		\textit{slip\_surf}[i].x - 
		\textit{slip\_surf}[i+1].x \leq 0)) \\
		\lor \lnot is\_on\_slope(\textit{slip\_surf}[0])\\
		\lor \lnot 
		is\_on\_slope(\textit{slip\_surf}[|\textit{slip\_surf}-1|])\\
		\lor \lnot is\_in\_slope(\textit{slip\_surf})\\
		\lor (cncvu() \land 
		\lnot (is\_concave\_up(\textit{slip\_surf})))\\
		\lor (obtu() \land 
		\lnot (has\_no\_sharp\_angles(\textit{slip\_surf})))\\
		\Rightarrow false\\
