...
enumerations, pointers (references), strings, structures, unions. These are
summarized in the following table.

\wss{I'm not sure we need pointers at this point.  An abstract design probably
  shouldn't have to invoke the concept of pointers.}
...
\noindent Finally, \progname uses two more important type-related concepts: void
and function pointers. Void is not a data type in itself; however, functions
that do not return any value are assigned a return type of void, and void
pointers, denoted by void*, are used for references to objects of an unspecified
data type. \progname also allows passing functions to other functions through
the use of function pointers, which hold references to function
definitions. Each function pointer is denoted by the name of their function type
and is defined by a specific function signature, such as:
...
\wss{I really don't think we need void in the abstract MIS.  If an access
  program doesn't return any value, we just don't assign it one.}
...
%%%%%%%%%%%%%%%%%%%% RIGID BODY MODULE %%%%%%%%%%%%%%%%%%%%%%%
\newpage \wss{put a newpage between all modules.}
...
\subsection{Uses} \label{SecUBody}
	\Shape, \Space, \Vector %\Transform, %\SpatialIndex, \Sequence
...
\subsubsection{State Variables} \label{SecSVBody}
	\textbf{BodyType} $\in$ \{DYNAMIC, STATIC\} \\ \wss{if you are going to
          create a new type, then this should be an exported type, not a state variable.}
	\newline
	\textbf{Body:} \wss{I don't know what this heading means?  You can just
...
%	bodyAlloc, or newBody, or newStaticBody are called before any 
%	other access program. 
	All input pointers are also assumed to be non-null.  \wss{This is far
