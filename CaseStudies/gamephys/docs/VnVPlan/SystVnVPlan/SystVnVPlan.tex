...
%Momentum before collision = $\mathbf{m_1}$${v_1}$ + $\mathbf{m_2}$${v_2}$ must be equal to:\\
%Momentum after collision = $\mathbf{m_1}$${v_1}$ + $\mathbf{m_2}$${v_2}$
...
\wss{The TBDs should be filled in.  I get the impression that some of the physics is giving you trouble.  The following resource looks pretty good
...
\olu{updated testcase, will test during system testing and see the trend of output result.}
...
\wss{Nice to see a usability test.  It is a bit simplistic, but that is fine for right now.}
\olu{updated TC12}
...
\wss{This isn't really complete.  For correctness, you can probably refer to your functional tests from the previous section.}\olu{updated}
...
\olu{survey will be looked into in future}
...
\olu{commenting out based on SRS feedback, Understandability and maintainability will not be tested in this phase.}
%\end{enumerate}
...
\begin{table}
	...
	\begin{tabular}{|c|p{5cm}|p{5cm}|}
		...
		\textbf{Testcase Number} & \textbf{Instance Models} & \textbf{Requirements}\\
		
		\hline 
		TC1-TC7& IM1 & R1, R2, R4, R5       \\ \hline
		TC8-TC9& IM2 & R1, R2, R4, R6   \\ \hline
		TC10-TC11& IM3, IM4 & R1, R3, R4, R7, R8   \\ \hline
		TC12& &NFR3 \\ \hline
		TC13& &NFR2 \\ \hline
		TC14& &NFR1 \\ \hline
...
~\newline
\wss{You can remove this section, since the details can be covered in Section \olu{removed section}
...
\wss{You should also think about parallel testing.  Calculate the answers for
  some more complex simulations with existing software and compare these results to your results.} \olu{will do, I need a lot of time to implement this library efficiently, I will consider parallel testing in future}
~\newpage
