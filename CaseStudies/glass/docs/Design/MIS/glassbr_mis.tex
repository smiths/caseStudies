...
calc\_j\_tol & inputs & $\mathbb{R}$ & -\\
calc\_pb & inputs & $\mathbb{R}$ & -\\
calc\_nfl & inputs & $\mathbb{R}$ & -\\
calc\_lr & inputs & $\mathbb{R}$ & -\\
calc\_is\_safePb & inputs & $\mathbb{R}$ & -\\
calc\_is\_safeLr & inputs & $\mathbb{R}$ & -\\
\hline
\end{tabular}
...
\noindent calc\_pb($j$) \textit{\# From SRS DD7 Dimensionless Load ($\hat{q}$)}
\begin{itemize}
\item output:  \textit{out} := $\frac{q(ab)^2}{Eh^4\text{GTF}}$
\item exception: None
\end{itemize}
