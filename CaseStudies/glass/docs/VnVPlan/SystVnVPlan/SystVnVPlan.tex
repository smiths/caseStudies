...
\title{Glass Breakage Analysis: System Verification and Validation Plan
  \wss{Include the name GlassBR here}} 
...
{https://github.com/smiths/caseStudies/tree/master/CaseStudies/glass}. \wss{Good!}
...
statically code review with Dr. Spencer Smith \wss{\LaTeX{} has a rule that it
  ...
  Smith (to force \LaTeX{} to not insert a line break).}  and the classmates in
CAS741 and dynamically by executing the test cases detailed in this plan and the
unit VnV plan using Pyton \wss{spell check!} testing frameworks.

\subsection{Software Validation Plan}

There is no validation plan for \progname{}.  \wss{You should the reason why
...
taken from the' Test Documentation' \wss{proof read!} for this project . Individual 
...
different forms of valid user input.  \wss{You should say where the values in
  Table 1 come from.}
...
$\hspace*{2cm}$\text{$\text{is\_safePb} = \text{True}$ and $\text{is\_safeLR} =
  ...
  these values make sense.} \wss{You don't need brackets around your numbers.}
...
%EndTestCase1
\newpage \wss{You don't normally need to hard code in newpages}
...
Tst\_ Pb\_ SmallDimensionValues \wss{I don't understand how the dimensions are
  small values.  They look like regular values.}
...
\wss{The input and output values for the above test cases do not seem to follow
  ...
  distinguish the names of any of your test cases based on this.}
...
\\
TC\refstepcounter{testnum}\thetestnum \label{TC_incorrectWEqUpprBndTest} & incorrectWEqUpprBndTest  & sdx = 130; sdy = 0; sdz = 0 & "Encountered an unexpected exception" 
\\
\hline
\end{tabular}
\end{adjustbox}
...
\wss{The error messages for the last test cases are all the same ``Encountered
  ...
  more descriptive error message, as for the other cases.}

\wss{Use ``uses'' to
  get the correct opening and closing quotes.}
...
Linux operating systems \wss{You can be more specific here.  My suggestion is
  that you explicitly say that all unit tests will be run on all environments.}
...
\wss{You can remove this section.}
...
\wss{Since the system tests for GlassBR are fairly simple, you should be looking
  ...
  and introduce this idea in the main body of your plan.}
